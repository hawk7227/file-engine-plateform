...
const CSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .chat-content { flex: 1; font-size: 15px; line-height: 1.6; color: var(--text-secondary); white-space: pre-wrap; padding-top: 4px; }
  ...
  .chat-content.streaming::after { content: '▋'; display: inline-block; width: 8px; height: 16px; background: var(--accent-primary); animation: blink 1s infinite; vertical-align: middle; margin-left: 4px; }
  ...
  return (
    ...
    <div className="app-container">
      ...
      <aside className="preview-panel">
        ...
        <div className="action-bar"><div className="dropdown"><button className="download-btn" onClick={e=>{e.stopPropagation();setDownloadMenuOpen(!downloadMenuOpen)}}>⬇️ Download ▾</button><div className={'dropdown-menu bottom '+(downloadMenuOpen?'show':'')}><button className="dropdown-item">📦 Download ZIP</button></div></div><div className="dropdown"><button className={'deploy-btn '+(isDeployed?'success':'')} onClick={e=>{e.stopPropagation();setDeployMenuOpen(!deployMenuOpen)}}>{isDeployed?'✓ Deployed':'🚀 Deploy ▾'}</button><div className={'dropdown-menu bottom '+(deployMenuOpen?'show':'')}><button className="dropdown-item" onClick={()=>handleDeploy('vercel')}>▲ Deploy to Vercel</button><button className="dropdown-item" onClick={()=>handleDeploy('github')}>🐙 Push to GitHub</button><div className="dropdown-divider"/><button className="dropdown-item" onClick={()=>handleDeploy('both')}>▲ + 🐙 Both</button></div></div></div></>}
      </aside>
