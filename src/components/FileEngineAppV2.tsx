  ...
  return (
    ...
    <div className="app-container">
      ...
      <aside className="preview-panel">
        ...
        <div className="action-bar"><div className="dropdown"><button className="download-btn" onClick={e=>{e.stopPropagation();setDownloadMenuOpen(!downloadMenuOpen)}}>⬇ Download ▾</button><div className={'dropdown-menu bottom '+(downloadMenuOpen?'show':'')}><button className="dropdown-item" onClick={async()=>{setDownloadMenuOpen(false);try{const{downloadAsZip}=await import('@/lib/export');const files=preview.files.map(f=>({path:f.path,content:f.content,language:f.path.split('.').pop()}));await downloadAsZip(files,currentProjectName);toast('success','Downloaded','ZIP saved')}catch (e: unknown){toast('error','Download Failed',(e instanceof Error ? e.message : String(e)))}}}> Download ZIP</button><button className="dropdown-item" onClick={async()=>{setDownloadMenuOpen(false);try{const{copyAllFilesAsText}=await import('@/lib/export');const files=preview.files.map(f=>({path:f.path,content:f.content}));await copyAllFilesAsText(files);toast('success','Copied','All files copied to clipboard')}catch (e: unknown){toast('error','Copy Failed',(e instanceof Error ? e.message : String(e)))}}}> Copy All Code</button></div></div><div className="dropdown"><button className={'deploy-btn '+(isDeployed?'success':'')} onClick={e=>{e.stopPropagation();setDeployMenuOpen(!deployMenuOpen)}}>{isDeployed?'✓ Deployed':' Deploy ▾'}</button><div className={'dropdown-menu bottom '+(deployMenuOpen?'show':'')}><button className="dropdown-item" onClick={()=>handleDeploy('vercel')}>▲ Deploy to Vercel</button><button className="dropdown-item" onClick={()=>handleDeploy('github')}> Push to GitHub</button><div className="dropdown-divider"/><button className="dropdown-item" onClick={()=>handleDeploy('both')}>▲ +  Both</button></div></div></div></>}
      </aside>
